...
class cnn(object):
    def __init__(self,rng,params,cost_function='mse',optimizer = RMSprop):
        ...
        #Layer2: conv2+pool
        subsample=(1,1)
        filter_shape=(72,p1.output_shape[1],3,3)
        c2=ConvLayer(rng, p1.output, filter_shape,p1.output_shape,border_mode,subsample, activation=nn.relu)
        p2=PoolLayer(c2.output,pool_size=pool_size,input_shape=c2.output_shape)

        #Layer3: conv2+pool
        filter_shape=(72,p2.output_shape[1],3,3)
        ...
        h2=HiddenLayer(rng,h1.output,512,2048,activation=nn.relu)

        h3=HiddenLayer(rng,h2.output,2048,4096,activation=nn.relu)

        lreg=LogisticRegression(rng,h3.output,4096,2048)
        self.output = lreg.y_pred

        self.params =c1.params+c2.params+c3.params+h1.params+h2.params+h3.params+lreg.params
